...
class CRMDropdownWrapper extends StatefulWidget {
  const CRMDropdownWrapper({super.key,
  ...
  State<StatefulWidget> createState() {
    return _CRMDropdownWrapperState();
  }
...
class _CRMDropdownWrapperState extends State<CRMDropdownWrapper> {
  ...
  void _checkHeight() {
    Future.delayed(const Duration(microseconds: 1), () {
      ...
      Offset overlayMaxPosition = Offset(
          _offset.dx + _size.width + _overlayWidth,
          _offset.dy + _size.height + _overlayHeight);

      double leftWidth = _offset.dx - 15;
      if (widget.dropdownDirection == CRMDropdownDirection.vertical) {
      ...
      if (widget.dropdownDirection == CRMDropdownDirection.right) {
        _overlayLeft = _size.width + widget.containerMargin.left + 5;
      } else if (widget.dropdownDirection == CRMDropdownDirection.bottom) {
        _overlayTop = _size.height + widget.containerMargin.top + 5;
      }else if (widget.dropdownDirection == CRMDropdownDirection.left) {
        _overlayLeft = -(_overlayWidth + 5);
      }else if (widget.dropdownDirection == CRMDropdownDirection.top) {
        _overlayTop = -(_overlayHeight + 5);
      }else if (widget.dropdownDirection == CRMDropdownDirection.horizontal ||
          widget.dropdownDirection == CRMDropdownDirection.vertical) {
        if (overlayMaxPosition.dx > screenSize.width) {
          if (leftWidth > rightWidth) {
            if (leftWidth > widget.dropdownMenuSize.maxWidth) {
              _overlayWidth = widget.dropdownMenuSize.maxWidth;
            } else if (_overlayWidth > leftWidth){ _overlayWidth = leftWidth;}

            if (widget.dropdownDirection == CRMDropdownDirection.vertical) {
              _overlayLeft =
                  widget.containerMargin.left - _overlayWidth + _size.width;
            } else if (widget.dropdownDirection ==
                CRMDropdownDirection.horizontal) {
        ...
        if (overlayMaxPosition.dy > screenSize.height) {
          if (bottomHeight > topHeight) {
            ...
            if (topHeight > widget.dropdownMenuSize.maxHeight) {
              _overlayHeight = widget.dropdownMenuSize.maxHeight;
            } else if (_overlayHeight > topHeight){ _overlayHeight = topHeight;}

            if (widget.dropdownDirection == CRMDropdownDirection.vertical) {
              _overlayTop = widget.containerMargin.top - _overlayHeight - 5;
            } else if (widget.dropdownDirection == CRMDropdownDirection.horizontal) {
              _overlayTop =
                  widget.containerMargin.top - _overlayHeight + _size.height;
            }
          }
        } else {
          if (widget.dropdownDirection == CRMDropdownDirection.vertical) {
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius:
                                widget.dropdownMenuStyle.borderRadius,
